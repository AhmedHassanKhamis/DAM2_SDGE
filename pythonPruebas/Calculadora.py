...
ventanaNombre = tk.Tk()
ventanaNombre.title("introduce tu nombre:")
ventanaNombre.geometry("300x75")

entry = ttk.Entry(ventanaNombre)
entry.place(x=50, y=25)
btn = ttk.Button(ventanaNombre, text="Enviar")
btn.place(x=200, y=25)


def lanzarCalculadora():
    nombreUsuario = entry.get()
    ventana = Toplevel(ventanaNombre)
    ventana.title("Calculadora de " + nombreUsuario)
...
btn.bind("<Button-1>", lambda event: lanzarCalculadora())
#ventana.mainloop()
